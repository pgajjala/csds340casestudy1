...
for k in range(1, 30):
    ...
    rbf_svm.fit(X_train_selected, y_train)

    # Predict the label using the select features only
    testOutputs = rbf_svm.predict_proba(X_test_selected)[:, 1]
...
param_grid = {'C': [0.01, 0.1, 1, 10, 100, 1000, 10000, 100000, 1000000], 'gamma': ['scale', 'auto']}
